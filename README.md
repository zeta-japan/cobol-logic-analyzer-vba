...
8. **別紙1ドラフト** — ソース全行 + 各分岐アームの先頭文に **ケースNo** を標記。
9. **パターン表ドラフト** — SECTION 順に全分岐を ①-1/①-2 で列挙、条件をテンプレート
   日本語化 (＝/≠/または/かつ)、処理行は PERFORM 直前コメントから要約、ケースNo 付き
   客先様式「別紙1」(コード＋ケース対応) への転記元
10. **分岐カバレッジ / 呼出関係 / 呼出関係図** — 分岐網羅集計・呼出関係表・呼出関係図

シングル `.xlsm` 内で完結し、外部ツール (.NET / Python 等) に依存しない。
社内および客先で **Excel + Windows 標準 COM のみ** が許可される環境向け。

### テストケース生成アルゴリズム (ver3.2)

全パス列挙は分岐数に対して指数爆発するため、**定向路径構造**でケースを直接構成する
(処理量は 分岐アーム数 × 路径長 に線形 — 1000 行・分岐 150+ のプログラムでも秒級):

1. **種子 2 本** — THEN 優先 / ELSE 優先の 2 walk (どちらも ABEND 回避) で大半のアームを回収
2. **スイープ** — 1 walk が未カバーのアームを最大限拾う経路を構成
   (分岐では未カバーの非 ABEND 可行アームを直取り、遠方のアームへは祖先重みで誘導)。
   これにより**ケース数は最少カバーに近い数**に収まる
3. **定向フォールバック** — 残ったアームのみ個別に経路構成
   (ABEND アーム、および定数伝播に阻まれるフラグ慣用法は「値転向」= 先行分岐の
   リテラル代入サイトを連鎖誘導して到達)

ループ型 PERFORM (VARYING / UNTIL / n TIMES / WITH TEST) の本体は 1 回インライン展開
(静的近似)。名前に ABEND を含む SECTION は自動で異常終了扱い。残った赤行 (未カバー) は
恒真/恒偽のデッドコード・到達不能アームなど**実情報**として機上確認に回す。
...
| ver3.1 定向路径構造 | ✅ 完了 | 枚挙→定向構成へ転換 (大規模プログラムの凍結解消)・ループ展開・値転向 |
| ver3.2 最少ケース化 | ✅ 完了 | スイープ走査 + 完全リプレイ。候補数/処理時間を大幅削減しつつ全カバレッジ維持 |
| ver3.3 XDM 成果物連携 | ✅ 完了 | ロジック階層(ソース順) TC列・分岐カバレッジ表 SECTION列・別紙1ドラフト・パターン表ドラフト (テンプレート日本語) |
| ver3.4 分岐カバレッジ表レイアウト | ✅ 完了 | SECTION帯 (A列名称 / C列備考)・正常=緑/異常=桃のケースヘッダ着色・SECTION単位の太枠・Meiryo UI |
| ver3.5 正常カバー二次パス | ✅ 完了 | 深層abend経路のループ境界アームを正常ケースで補完し、正常カバレッジを100%化 |
| ver3.6 旧カバレッジシート再集計 | ✅ 完了 | 旧「分岐カバレッジ」シートを flow エンジンで再集計し、分岐カバレッジ表と数値一致 |
| ver3.7 伝播性ターミネータ検出 | ✅ 完了 | 無条件に終了段へ至る SECTION を不動点で terminator 化し、深層abend カスケードによる「経路構築不可」を解消 (3.7.1: 初期化順バグ修正) |
| ver3.8 デッドセクション除外 | ✅ 完了 | 未PERFORM のデッドセクションを灰「対象外」表示・カバー率分母から除外 (到達可能な未カバーは赤のまま) |
| ver3.9 孤立SECTIONの単体テスト化 | ✅ 完了 | メインフローが PERFORM しない孤立SECTION (orphan) を専用エントリとして単体テスト化し、各分岐に TC を付与 |
| ver3.10 終端なしエントリの暗黙正常終了 | ✅ 完了 | GOBACK を持たず末尾 EXIT で抜けるエントリを暗黙の正常終了とみなし正常ケースを生成 |
