...
' line number -> enclosing inline-PERFORM loop depth (for body indentation
' in the pattern draft; rebuilt per BuildPatternDraft run)
Private mLoopDepth As OrderedDict
...
Public Sub BuildPatternDraft(ByVal flowR As OrderedDict, ByVal src As String)
    ...
    If flowR.Exists("arms") Then
        For Each da In flowR.Item("arms")
            ...
            ' diag; the tcMap guard stops it being mislabeled 対象外 here
    ...
    Set mLoopDepth = BuildLoopDepth_(norm.Item("Lines"))
    ' output (LINKAGE) records so the MOVE filter keeps final-value writes
    Set mOutPrefixes = Nothing
    On Error Resume Next
    Set mOutPrefixes = CobolCalls.Get_ProcedureUsing(norm.Item("Lines"))
    On Error GoTo 0
...
Private Sub EmitAction_(ByVal ws As Worksheet, ByRef row As Long, ByVal n As OrderedDict, _
    ...
    ws.Cells(row, 3).Value = Indent_(depth + LoopDepthAt_(CLng(n.Item("startLine")))) & jp
    row = row + 1
End Sub
...
Private Sub RenderArm_(ByVal ws As Worksheet, ByRef row As Long, ByVal num As String, _
    ...
    ws.Cells(row, 3).Value = Indent_(depth + lpd) & condText
    ...
    If row = rowBefore Then
        ' nothing emitted: distinguish a genuinely empty/CONTINUE arm from
        ' one whose statements were all suppressed as housekeeping
        Dim emptyMsg As String
        If children Is Nothing Then
            emptyMsg = "（処理なし）"
        ElseIf children.Count = 0 Then
            emptyMsg = "（処理なし: CONTINUE）"
        Else
            emptyMsg = "（後処理のみ・検証Point対象外）"
        End If
        ws.Cells(row, 3).Value = Indent_(depth + 1 + lpd) & emptyMsg
...
Private Function ActionJp_(ByVal n As OrderedDict, ByVal cmts As OrderedDict) As String
    ...
    ln = CLng(n.Item("startLine"))
    ' housekeeping (INITIALIZE / GOBACK / STOP RUN / EXIT[ PROGRAM] /
    ' CONTINUE) is not a 検証Point - skip it so the draft stays test-focused
    If IsHousekeeping_(lbl) Then Exit Function
    ...
    If Left$(lbl, 6) = "GO TO " Then
        ...
        Exit Function
    End If
    ' kept output MOVEs (IsOutputMove_ = True) reach here to render
...
' statements that are not 検証Points（初期化・終了・無処理、および出力で
' ない MOVE）。パターン表ドラフトでは出力しない。MOVE は出力(LINKAGE)
' 項目への代入だけ残す = 検証で確認する値の設定だから。初期化や CALL 前
' のサブパラメータ設定 MOVE は除外する。
Private Function IsHousekeeping_(ByVal lbl As String) As Boolean
    IsHousekeeping_ = False
    If lbl = "EXIT" Or lbl = "CONTINUE" Or lbl = "GOBACK" _
        Or lbl = "INITIALIZE" Or Left$(lbl, 11) = "INITIALIZE " _
        Or Left$(lbl, 8) = "STOP RUN" Or Left$(lbl, 12) = "EXIT PROGRAM" Then
        IsHousekeeping_ = True
        Exit Function
    End If
    If Left$(lbl, 5) = "MOVE " Then
        IsHousekeeping_ = Not IsOutputMove_(lbl)
    End If
End Function

' True if a MOVE writes to a program-OUTPUT item. Output items = the
' PROCEDURE DIVISION USING operands (the LINKAGE records) and their
' COPY-prefixed fields (<operand>-...). With no USING list every MOVE is
' treated as housekeeping (the conservative, pre-refinement behavior).
Private Function IsOutputMove_(ByVal lbl As String) As Boolean
    IsOutputMove_ = False
    If mOutPrefixes Is Nothing Then Exit Function
    Dim p As Long, tgts As String
    p = InStr(lbl, " TO ")
    If p = 0 Then Exit Function
    tgts = Trim$(Mid$(lbl, p + 4))
    If Right$(tgts, 1) = "." Then tgts = Trim$(Left$(tgts, Len(tgts) - 1))
    Dim toks() As String, i As Long, t As String, pre As Variant
    toks = Split(tgts, " ")
    For i = LBound(toks) To UBound(toks)
        t = UCase$(Trim$(toks(i)))
        If Len(t) > 0 Then
            For Each pre In mOutPrefixes
                If t = UCase$(CStr(pre)) Or Left$(t, Len(CStr(pre)) + 1) = UCase$(CStr(pre)) & "-" Then
                    IsOutputMove_ = True
                    Exit Function
                End If
            Next pre
...
' enclosing inline-PERFORM loop depth at a source line (0 = not in a loop)
Private Function LoopDepthAt_(ByVal lineNo As Long) As Long
    LoopDepthAt_ = 0
    If mLoopDepth Is Nothing Then Exit Function
    If mLoopDepth.Exists(CStr(lineNo)) Then LoopDepthAt_ = CLng(mLoopDepth.Item(CStr(lineNo)))
End Function

' scan the normalized lines: each line records the count of inline-PERFORM
' loops enclosing it. A loop header (PERFORM UNTIL/VARYING/WITH TEST/n
' TIMES, no out-of-line paragraph) opens a level; END-PERFORM closes it.
Private Function BuildLoopDepth_(ByVal lines As Collection) As OrderedDict
    Dim m As OrderedDict
    Set m = New OrderedDict
    Set BuildLoopDepth_ = m
    If lines Is Nothing Then Exit Function
    Dim le As OrderedDict, txt As String, depth As Long, ln As Long
    depth = 0
    For Each le In lines
        ln = CLng(le.Item("Number"))
        txt = UCase$(Trim$(CStr(le.Item("Text"))))
        If Left$(txt, 11) = "END-PERFORM" Then
            If depth > 0 Then depth = depth - 1
            If Not m.Exists(CStr(ln)) Then m.Add CStr(ln), depth
        Else
            If Not m.Exists(CStr(ln)) Then m.Add CStr(ln), depth
            If IsInlineLoopHead_(txt) Then depth = depth + 1
        End If
    Next le
End Function

' is this normalized line an INLINE PERFORM loop header (body + END-PERFORM)?
Private Function IsInlineLoopHead_(ByVal txt As String) As Boolean
    IsInlineLoopHead_ = False
    If Left$(txt, 8) <> "PERFORM " Then Exit Function
    Dim rest As String
    rest = Trim$(Mid$(txt, 9))
    If Right$(rest, 1) = "." Then rest = Trim$(Left$(rest, Len(rest) - 1))
    If Left$(rest, 6) = "UNTIL " Or Left$(rest, 8) = "VARYING " Or Left$(rest, 10) = "WITH TEST " Then
        IsInlineLoopHead_ = True
        Exit Function
    End If
    ' inline 'PERFORM <count> TIMES' = exactly two tokens, last is TIMES
    ' (numeric OR data-name count); out-of-line 'PERFORM <para> <count>
    ' TIMES' has three tokens and no END-PERFORM, so it must NOT count
    Dim toks() As String
    toks = Split(rest, " ")
    If UBound(toks) = 1 Then
        If UCase$(toks(1)) = "TIMES" Then IsInlineLoopHead_ = True
    End If
End Function
...
' no output records -> every MOVE is housekeeping)
...
' test shim with output records (comma-separated USING operands) so the
' MOVE keep/skip refinement can be pinned
Public Function ActionJpOf2(ByVal label As String, ByVal outCsv As String) As String
    Dim pr As Collection, v As Variant
    Set pr = New Collection
    For Each v In Split(outCsv, ",")
        If Len(Trim$(CStr(v))) > 0 Then pr.Add Trim$(CStr(v))
    Next v
    Set mOutPrefixes = pr
    Dim n As OrderedDict
    Set n = New OrderedDict
    ...
    n.Add "label", label
    n.Add "startLine", 1
    ActionJpOf2 = ActionJp_(n, New OrderedDict)
    Set mOutPrefixes = Nothing
End Function
